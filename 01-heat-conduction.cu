...
{
  float d2tdx2, d2tdy2;
  int i00 =  blockIdx.x * 256 + threadIdx.x;
  if (i00 < ni*(nj-1) && i00 > ni && i00 % ni !=0 && i00 % ni != (ni - 1))
  {
      int im10 = i00 - 1;
      int ip10 = i00 + 1;
      int i0m1 = i00 - ni;
...
int main()
{
  ...
  int blockNum = (ni * nj + 255) / 256;
  for (istep=0; istep < nstep; istep++) {
    step_kernel_mod<<<blockNum,256>>>(ni, nj, tfac, temp1, temp2);
